...
import { getAllRegisteredAgents, approveAgentProfile } from '../../../services/AdminServices';
...
export const AddAgents = () => {
  ...
  const handleFormSubmit = async (e) => {
    ...
    try {
      ...
      await approveAgentProfile(customerId, isApproved);
      fetchRegisteredCustomers()
      successToast("Agent has been activated successfully!");
      setCustomerId('');
    }
    catch (error) {
      if (error.response?.data?.message || error.specificMessage) {
        ...
        errorToast("An error occurred while Activating agent.");
      }
    }
  }
  ...
  const fetchRegisteredCustomers = async () => {
    try {
      ...
      const response = await getAllRegisteredAgents(formData);
      console.log("in active customer click", response?.data.content ?? [])
      setData(response?.data ?? []);
      console.log("data in response", data)

      setKeysToBeIncluded(["agentId", "firstName", "dateOfBirth", "qualification"]);
  ...
  return (
    <div className='content-area'>
      <AreaTop pageTitle={"Registered agents"} pagePath={"new-agents"} pageLink={`/admin/dashboard/${routeParams.id}`} />
      <section className='content-area-form'>
        <div className="admin-form">
          <div className="data-info">
            <h3 className="data-table-title">New Registered Agents</h3>
          </div>
          <div className='activate-form'>
            <form>
              ...
              <button type="submit" className="form-submit-form" onClick={(event) => handleFormSubmit(event)}>Approve Agent</button>
            </form>
          </div>
      ...
      {(showActiveCustomers || showInactiveCustomers) && (
        <section className="content-area-table">
          <div className="data-table-info">
            <h3 className="data-table-title">Registered Agents</h3>
          </div>
