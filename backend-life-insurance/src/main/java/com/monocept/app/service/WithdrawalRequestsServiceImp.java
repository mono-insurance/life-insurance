...
public class WithdrawalRequestsServiceImp implements WithdrawalRequestsService {
    ...
    public WithdrawalRequestsDTO createWithdrawalRequestForCustomer(WithdrawalRequestsDTO withdrawalRequestDTO) {
        ...
        emailDTO.setTitle("Withdrawal request created");
        emailDTO.setBody("Congrats!! you have created a request to claim the policy.\n" +
                " Now, our employees will verify your request, it could take some time");
    ...
    public WithdrawalRequestsDTO createWithdrawalRequestForAgent(WithdrawalRequestsDTO withdrawalRequestDTO) {
        ...
        emailDTO.setTitle("Withdrawal request created");
        emailDTO.setBody("Congrats!! you have created a request to claim your commission.\n" +
                " Now, our employees will verify your request, it could take some time");
        emailService.sendAccountCreationEmail(emailDTO);
    ...
    public Boolean approveOrRejectRequest(Long withdrawalRequestId, Boolean isApproved) {
        ...
        if (withdrawalRequest.getAgent() != null)
            emailDTO.setEmailId(withdrawalRequest.getAgent().getCredentials().getEmail());
        else emailDTO.setEmailId(withdrawalRequest.getCustomer().getCredentials().getEmail());
        if (isApproved) {
            withdrawalRequest.setIsApproved(true);
            withdrawalRequestRepository.save(withdrawalRequest);
            emailDTO.setTitle("Withdrawal request approved");
            emailDTO.setBody("Congrats!! your Withdrawal request has been approved");
        } else {
            withdrawalRequestRepository.delete(withdrawalRequest);
            emailDTO.setTitle("Withdrawal request not approved");
            emailDTO.setBody("Oops!! your request has not been approved, please contact us if it is a mistake");
        }
        emailService.sendAccountCreationEmail(emailDTO);
        return false;
    }
